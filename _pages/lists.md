...
The `onChangedHandler` is used to trigger a rerender whenever the checkbox `selectInactive` is changed. The rerender triggers the listGrid to requery the data.

`filterBinding` is the binding in the query `qUser`.

`valueBinding` is the binding in the view.

The `filterParameter` above has the effect of adding to the `where` clause generated by the persistence pipeline, e.g. if the value of `valueBinding` (i.e. the attribute 'selectInactive') is 'true' then `and inactive = true` is applied as an additional filter to the query `qUser`.
